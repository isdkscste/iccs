...
<div class="form-group row">
  ...
  <div class="col-sm-6 err">
    <input type="text"  class="form-control" id="email" name="email" readonly value="<?php echo e(Auth::user()->email); ?>"     >
...
<?php
                            ...
                            $enddate = date('2023-02-28 16:59:00');
                            $now = date('Y-m-d H:i:s'); 
                            ?>
                            <?php if($now<$enddate): ?>
...
$('[data-mask]').inputmask();
  ...
  $.ajax({
    ...
    success :function(result){
      ...
      // if(result[0].qualification =="mtech")
      // {
      //   //alert(result[0].qualification);
      ...
      else
      {
       ...
       $('#Ref_email3').val(result[0].ref_email3);
        ...
        if(result[0].submit==1)
        {
          ...
          $('#appl_view2').show();
          $('#v1').text(result[0].id);
          var ur = '<?php echo e(route("generatePDF", ":pd")); ?>';
...
{ 
...

  {
   ...
   depends: function(element) {
    if (($('#post_code').val() == "SCC/02/2020" || $('#post_code').val() == "SCC/03/2020" || $('#post_code').val() == "SCC/07/2020" || $('#post_code').val() == "SCC/08/2020" || $('#post_code').val() == "SCC/09/2020" || $('#post_code').val() == "SCC/10/2020") &&($('#hid_phd2').val()=="")) 
    {
      return true;
    } 
    else 
...
function age_cal()
{
    ...
    //alert("cdf");
    var v=$('#dob').val();
    var now = moment('2021-02-13','YYYY-MM-DD');
    var birthDate = moment(v,'YYYY-MM-DD');
...
function duration1()
{
  ...
  var From_date1=moment(From_date,'YYYY-MM-DD');
  var To_date1=moment(To_date,'YYYY-MM-DD');
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH F:\xampp\htdocs\cwrdm\resources\views/scientist/application.blade.php ENDPATH**/ ?>